...
- 名單優先來源為 `https://apis.youbike.com.tw/json/station-yb2.json`，雙北公開站 `status=1/2` 均保留。若 GitHub 主機遇到403或連線失敗，自動改用已授權的 TDX Station 與 Availability 介面，分頁讀取全站並合併實際服務狀態。官方名稱、車位數可更新，但不覆寫 CPS 分析或瀏覽器中的手動模擬。
- 既有責任區沿用 CPS 匯出資料；公開名單沒有 CPS 責任區欄位，新站暫列「待設定」，不以行政區猜測。
- TDX Station 沒有行政區欄位：既有行政區保留 CPS 值，新站尚無來源時列「待確認」，不從不完整地址推測。日後 CPS 資料補入時，自動補齊待確認欄位，不覆蓋已知責任區。
